...
if __name__ == '__main__':
    ...
    ESTABELECIMENTOS = EXTRATOR_CNPJ(baixar_e_extrair=False, nome_arquivo="Estabelecimentos").run()

    print(f'Termino da coleta dos ESTABELECIMENTOS: {strftime("%d/%m/%Y %H:%M:%S", localtime())}')
    EMPRESAS = EXTRATOR_CNPJ(baixar_e_extrair=False, nome_arquivo="Empresas").run()

    print(f'Termino da coleta dos EMPRESAS: {strftime("%d/%m/%Y %H:%M:%S", localtime())}')
    MUNICIPIOS = EXTRATOR_CNPJ(baixar_e_extrair=False, nome_arquivo="Municipios").run()

    print(f'Final da coleta dos MUNICIPIOS: {strftime("%d/%m/%Y %H:%M:%S", localtime())}')
    CNAES = EXTRATOR_CNPJ(baixar_e_extrair=False, nome_arquivo="Cnaes").run()
